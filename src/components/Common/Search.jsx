...
const SearchBar = () => {
    ...
    const handleSearch =(e)=>{
        ...
        dispatch(fetchProductsByFilters({search:serachTerm}))
        navigate(`/collections/all?search=${serachTerm}`)
        setIsOpen(false);
    }
    return (
            ...
            {isOpen ? (
                <form onSubmit={handleSearch} className='relative flex items-center justify-center w-full'>
                    <div className='relative w-1/2'>
                        <input type="text" placeholder='Search' value={serachTerm} onChange={(e) => setSerachTerm(e.target.value)} className='bg-gray-100 px-4 py-2 pl-2 pr-12 rounded-lg focus:outline-none w-full placeholder:text-gray-700' />
